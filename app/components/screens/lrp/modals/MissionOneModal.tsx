...
const MissionOneModal = ({ close }) => {
	const [width, height] = useWindowSize()
	const scale = width > 767.98 ? Math.min(width / 978, (height - 140) / 740) : 1
				...
				<Wrapper $fullWidth>
					...
					<GoalsWrapper $flex>
						<Goal $first>
							...
							<GoalText>
								<b>
									СОЗДАНИЕ СРЕДСТВ, ЭФФЕКТИВНЫХ В СНИЖЕНИИ ДИСКОМФОРТА КОЖИ,{' '}
								</b>{' '}
						...
						<Goal $second>
							<Background>
								...
								<GoalText>
									<b>
										ПОВЫШЕНИЕ ЗНАЧИМОСТИ ВОПРОСА СОСТОЯНИЯ КОЖИ ПРИ ОНКОЛОГИИ В
						...
						<Goal $third>
							...
							<GoalText>
								<b>ПОВЫШЕНИЕ ОСВЕДОМЛЕННОСТИ И ПРОФИЛАКТИКИ РАКА КОЖИ</b>
							</GoalText>
						</Goal>
					</GoalsWrapper>
				</Wrapper>
...
export default MissionOneModal
