...
const DayFiveSchedule = forwardRef<HTMLDivElement, any>(({ close }, ref) => {
	const { getScheduleCurrentState } = useScheduleCurrentState()
	const daySchedule = useScheduleSelector('2023-12-04')
	const status = getScheduleCurrentState('2023-12-04')
				...
				<Schedule.Content>
					<Schedule.Header>
						<Schedule.HeaderImage>
							<picture>
								<source
									srcSet={topImage('day-5/top-664')}
									media={`(min-width: ${sizes.phone_xl}px)`}
								/>
								<img src="#" srcSet={topImage('day-5/top')} />
							</picture>
						</Schedule.HeaderImage>
...
export default DayFiveSchedule
