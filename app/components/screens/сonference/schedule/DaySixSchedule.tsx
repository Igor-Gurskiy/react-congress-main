...
const daySchedule = schedule.get('2023-12-05')

const DaySixSchedule = forwardRef<HTMLDivElement, any>(({ close }, ref) => {
	const { getScheduleCurrentState } = useScheduleCurrentState()
	const daySchedule = useScheduleSelector('2023-12-05')
	const status = getScheduleCurrentState('2023-12-05')

	const topImage = (name, ext = 'jpg') =>
		`/assets/images/schedule/${name}.${ext} 1x, assets/images/schedule/${name}@2x.${ext} 2x`

	useEffect(() => {
		API.tracker.sendEvent(EventsEnum.confDay6)
	}, [])
				...
				<Schedule.Content>
					<Schedule.Header>
						<Schedule.HeaderImage>
							<picture>
								<source
									srcSet={topImage('day-6/top-664')}
									media={`(min-width: ${sizes.phone_xl}px)`}
								/>
								<img src="#" srcSet={topImage('day-6/top')} />
							</picture>
						</Schedule.HeaderImage>
						...
						<div className="text">
							<h1>
								...
								<span className="title2">Фармация</span>
							</h1>
						</div>
					</Schedule.Header>
