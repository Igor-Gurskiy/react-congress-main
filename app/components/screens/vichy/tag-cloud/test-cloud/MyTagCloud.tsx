...
class TagCloud extends React.Component<ITagCloudProps, ITagCloudState> {
	...
	public componentDidMount() {
		this.mounted = true
		this.setState({ width: 1200, height: 300 })
	}
	...
	public calculateLayout(
		...
		return new Promise((resolve) => {
			res.on('end', (items: any) => {
				const newChildren = items.map((item: any, index: number) => {
					let x = item.x + item.x0 + width / 2
					let y = item.y + item.y0 + height / 2
					...
					const newStyle = {
						...
						whiteSpace: 'nowrap',
						width: item.width,
						...(style.color && typeof style.color === 'function'
							? { color: style.color(item.child, index) }
							: {}),
					}
